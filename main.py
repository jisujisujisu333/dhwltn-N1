...
# 페이지 설정
st.set_page_config(
    page_title="서울 최저기온과 최고기온 관계",
    page_icon="🌡️",
    layout="wide"
)

st.title("🌡️ 서울의 일별 최저기온과 최고기온의 관계")
st.write("1926년부터 2025년까지 일별 최저기온과 최고기온의 관계를 산점도로 확인합니다.")
...
ax.set_title("서울의 일별 최저기온과 최고기온의 관계", fontsize=18)
ax.set_xlabel("최저기온 (℃)", fontsize=12)
ax.set_ylabel("최고기온 (℃)", fontsize=12)
ax.grid(True, alpha=0.3)

st.pyplot(fig)

# 상관계수 계산
correlation = df_100["최저기온"].corr(
    df_100["최고기온"]
)

# 정보 표시
st.subheader("📊 기온 관계 분석")

col1, col2, col3 = st.columns(3)

col1.metric(
    "최저기온 평균",
    f"{df_100['최저기온'].mean():.1f} ℃"
)

col2.metric(
    "최고기온 평균",
    f"{df_100['최고기온'].mean():.1f} ℃"
)
...
st.write(
    "상관계수가 1에 가까울수록 "
    "최저기온이 높을 때 최고기온도 함께 높아지는 경향이 강하다는 의미입니다."
)
